...
import { Vector2, Vector3, Color, DoubleSide } from 'three'
import {
  Emitter,
  InstancedParticles,
  useParticleLifetime,
...
const amount = Array(100).fill(0).map(() => new Vector2(between(-10, 10), between(0, 10)))
...
const ParticleRising = () => {
  ...
  useFrame(() => {
    group.current.children.forEach(o => {
      ...
      const screenspaceHeight = (o.position.clone().project(camera).y + 1) * 0.5
  ...
  return (
    <InstancedParticles capacity={5_000}>
      ...
      <group ref={group}>
        {amount.map((o, i) => {
          return (
            <mesh key={i} position={[o.x, Math.random() * maxHeight, o.y]}>
              <sphereGeometry args={[0.01]} />
              <meshBasicMaterial color={new Color("#48CCD0").multiplyScalar(2.5)} toneMapped={false} />

              <Emitter
                rate={Math.random() * 15 + 5}
                setup={({ mesh, position }) => {
                  position.add(tmpVec3.randomDirection().multiplyScalar(upTo(0.05)))
